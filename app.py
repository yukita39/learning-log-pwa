...
def redis_health():
    ...
    try:
        ...
        if redis_url == 'memory://':
            result['status'] = 'memory-only'
            result['details']['message'] = 'メモリストレージを使用中'
        else:
            ...
            result['details']['memory'] = {
                ...
                'total_system_mb': round(memory.get('total_system_memory', 0) / 1024 / 1024, 2)
            }
            
            # レート制限のテスト
            test_key = f'health_check:{datetime.now().timestamp()}'
            r.setex(test_key, 60, 'test')
    ...
    # ステータスコード
    status_code = 200 if result['status'] in ['connected', 'memory-only'] else 503
